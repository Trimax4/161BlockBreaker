...
int Physic::getCircleCollision(std::vector<Entity*> entities, int nextX, int nextY)
{
	int xCenter = (nextX + ballWidth / 2); //get the x value of center of circle
	int yCenter = (nextY + ballHeight / 2); //get the y value of the center of circle
	int closestX;
	...
	for (int i = 0; i < entities.size(); i++)
	{
		if (entities[i] == ball || entities[i] == nullptr)
		{
			continue;
		}
		if (xCenter < entities[i]->getX())
...
bool Physic::hitBrickBottom(int brickY, int ballY)
{
	return brickY <= ballY && brickY + 4 >= ballY;
}

bool Physic::hitBrickTop(int brickY, int ballY)
{
	return brickY >= ballY && brickY - 24 <= ballY;
}

void Physic::changeXSpeed(double* x, double newSpeed)
{

}

void Physic::changeYSpeed(double* y, double newSpeed)
{
